...
<textarea readonly style = "width:400px;margin-right:26cm"> 
	Step{{$step->step_number ?? '?'}}: {{$step->script ?? ' '}}
</textarea>

<form method="post" action="{{route('/scriptDownload')}}" enctype="multipart/form-data">
                            @csrf
...
<button class="btn btn-danger" type="submit" style="padding: 5px 10px; margin-left:850px;margin-top:-5cm">
                ...
                <img src="{{asset('assets/images/download.png')}}" width="25" height="25">Download Script</span></button>
</form>
...
<script>
	...
	let last_end_{{$step->step_number ?? '_'}} = Date.now();
	...
	vid_{{$step->step_number ?? '_'}}.onsuspend = function() {
		...
		session_count_{{$step->step_number ?? '_'}}_suspend++;
	...
	function sendTimedEvent_{{$step->step_number ?? '_'}}(function_now, last_time, last_suspended, type, session_counter, meta_data_maker) {
		...
		let suspended_delta = -last_suspended;
		if (last_suspended < 0) suspended_delta += function_now;
		...
		sendInteraction(type, function_now, time_delta, event_count, session_count, {{$step->id}}, {{$step->tutorial_id}}, "step_video_{{$step->step_number ?? ''}}", "{{$step->video}}", meta_data_maker());
		
		console.log("Video #{{$step->step_number ?? '?'}} event #"+event_count+" = "+type+" (#"+session_count+"), dt="+time_delta+", ds="+suspended_delta);
	}
	
	function makeOnStart_{{$step->step_number ?? '_'}}(type, session_counter, meta_data_maker) {
		return () => {
			var function_now = last_start_{{$step->step_number ?? '_'}} = Date.now();
			var last_suspended = suspended_{{$step->step_number ?? '_'}};
			suspended_{{$step->step_number ?? '_'}} = 0; // no longer suspended
			
			sendTimedEvent_{{$step->step_number ?? '_'}}(function_now, last_end_{{$step->step_number ?? '_'}}, last_suspended, type, session_counter, meta_data_maker);
		};
	}
	
	function makeOnEnd_{{$step->step_number ?? '_'}}(type, session_counter, meta_data_maker) {
		return () => {
			var function_now = last_end_{{$step->step_number ?? '_'}} = Date.now();
			var last_suspended = suspended_{{$step->step_number ?? '_'}};
			suspended_{{$step->step_number ?? '_'}} = 0; // no longer suspended
			
			sendTimedEvent_{{$step->step_number ?? '_'}}(function_now, last_start_{{$step->step_number ?? '_'}}, last_suspended, type, session_counter, meta_data_maker);
		};
	}
	
	function makeOnContinue_{{$step->step_number ?? '_'}}(type, session_counter, meta_data_maker) {
		return () => {
			var function_now = Date.now();
			var last_suspended = suspended_{{$step->step_number ?? '_'}};
	...
	vid_{{$step->step_number ?? '_'}}.onratechange = makeOnContinue_{{$step->step_number ?? '_'}}('ratechange',
		...
		() => { return {
			video_id: "step_video_{{$step->step_number ?? ''}}",
			playback_rate:vid_{{$step->step_number ?? '_'}}.playbackRate,
			error: vid_{{$step->step_number ?? '_'}}.muted
		}});
